...
try:
	...
	try:
		if os.name == 'posix':
			os.system('pip3 install colorama termcolor requests ipaddress')
			sys.exit('[+] I have installed nessecary modules for you')
		elif os.name == 'nt':
			os.sytem('c:\python27\scripts\pip.exe install colorama requests ipaddress termcolor')
			sys.exit('[+] I have installed nessecary modules for you')
		else:
	...
	if args.Synflood:
		...
		while 1:
			if args.i == False:
				...
				ip = args.i
			try:
				for x in list(range(0,int(args.T))):
